...
const ChatDashboard = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const res = await axios.get("https://chat-backend-1-twvb.onrender.com/api/chat/history", {
      ...
      try {
        ...
        if (user && user.id) {
          const res = await axios.get(`https://chat-backend-1-twvb.onrender.com/api/user/profile`, {
            headers: { Authorization: `Bearer ${token}` },
          });
